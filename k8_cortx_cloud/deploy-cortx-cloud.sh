...
##########################################################
# CORTX 3rd party deploy functions
##########################################################
...
deployConsul $storage_class $num_worker_nodes
deployOpenLDAP $num_openldap_replicas
deployZookeeper $num_worker_nodes $storage_class
deployKafka $num_worker_nodes $storage_class
...
deployCortxGlusterFS $node_name_list $node_selector_list $gluster_vol $gluster_pv_name $gluster_pvc_name $gluster_etc_path $namespace
deleteStaleAutoGenFolders $node_name_list
deployCortxConfigMap $cfgmap_path $node_name_list $namespace
deployCortxSecrets $cfgmap_path $namespace
deployCortxControlProvisioner $gluster_pv_name $gluster_pvc_name $shared_storage $local_storage $namespace
deployCortxDataProvisioner $node_selector_list $node_name_list $node_name $gluster_pv_name $gluster_pvc_name $shared_storage $local_storage $namespace
deployCortxControl $local_storage $shared_storage $namespace
deployCortxData $node_selector_list $node_name_list $node_name $gluster_pv_name $shared_storage $local_storage $cvg_index_list $namespace
deployCortxServices $namespace
deployCortxSupport $node_name_list $local_storage $shared_storage $namespace
cleanup